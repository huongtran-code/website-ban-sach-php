...
if (isset($_POST['promotion_id']) && isset($_POST['updates'])) {
    ...
    if (!is_array($updates) || empty($updates)) {
        header("Location: ../edit-promotion.php?id=$promotion_id&error=Không có dữ liệu để cập nhật");
        exit;
    }
    ...
    if (!$promotion) {
        header("Location: ../edit-promotion.php?id=$promotion_id&error=Chương trình không tồn tại");
        exit;
    }
    ...
    try {
        ...
        if ($error_count > 0) {
            header("Location: ../edit-promotion.php?id=$promotion_id&success=Cập nhật $success_count sách thành công, $error_count sách lỗi");
        } else {
            header("Location: ../edit-promotion.php?id=$promotion_id&success=Cập nhật thành công $success_count sách");
        }
