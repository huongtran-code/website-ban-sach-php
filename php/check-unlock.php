...
if ($lock_status['status'] == 'ok') {
    ...
    header("Location: ../index.php?success=" . urlencode("Tài khoản đã được mở khóa!"));
    ...
    if ($current_balance >= $total_penalty) {
        ...
        try {
            ...
            header("Location: ../index.php?success=" . urlencode("Đã thanh toán " . number_format($total_penalty) . "đ. Tài khoản đã được mở khóa!"));
        } catch (Exception $e) {
            $conn->rollBack();
             header("Location: ../locked.php?error=" . urlencode("Lỗi xử lý: " . $e->getMessage()));
        }
    } else {
        // Still not enough
        ...
        $_SESSION['locked_balance'] = $current_balance;
         ...
         header("Location: ../locked.php?error=" . urlencode("Số dư không đủ để thanh toán phí phạt (" . number_format($total_penalty) . "đ). Vui lòng nạp thêm " . number_format($shortfall) . "đ."));
    }
}
