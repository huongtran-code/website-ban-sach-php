...
if ($has_hardcopy_for_address) {
    ...
    if (!empty($city_id) && !empty($district_id) && !empty($ward_id)) {
        ...
        if (empty($house_number)) {
            header("Location: ../checkout.php?error=Vui lòng nhập số nhà / số tầng");
            exit;
        }
        // Sử dụng địa chỉ đầy đủ từ hidden input (đã được JS tạo)
        if (empty($default_address)) {
            header("Location: ../checkout.php?error=Vui lòng điền đầy đủ địa chỉ giao hàng");
            exit;
        }
    } else {
        // Fallback: kiểm tra textarea cũ (nếu có)
        if (empty($default_address)) {
            header("Location: ../checkout.php?error=Vui lòng điền đầy đủ địa chỉ giao hàng khi chọn sách bản cứng");
            exit;
        }
    }
}
...
if ($payment_method === 'cod' && !$all_hardcopy) {
    header("Location: ../checkout.php?error=Thanh toán khi nhận hàng (COD) chỉ áp dụng khi tất cả sách đều là bản cứng");
    exit;
}
...
try {
    ...
    if (!empty($coupon_code)) {
        ...
        if ($coupon) {
            ...
            if (!$check_result['applicable']) {
                $conn->rollBack();
                header("Location: ../checkout.php?error=" . urlencode($check_result['message']));
                exit;
            }
            ...
            if (isset($coupon['usage_limit']) && $coupon['usage_limit'] !== null) {
                ...
                if ($usage_count >= $coupon['usage_limit']) {
                    $conn->rollBack();
                    header("Location: ../checkout.php?error=" . urlencode('Mã ' . $coupon_code . ' đã hết lượt sử dụng'));
                    exit;
                }
            }
            ...
            if (isset($coupon['max_usage_per_user']) && $coupon['max_usage_per_user'] !== null) {
                ...
                if ($user_usage >= $coupon['max_usage_per_user']) {
                    $conn->rollBack();
                    header("Location: ../checkout.php?error=" . urlencode('Bạn đã sử dụng mã ' . $coupon_code . ' đủ ' . $coupon['max_usage_per_user'] . ' lần'));
                    exit;
                }
            }
        }
    ...
    if ($payment_method === 'balance' && $order_total > 0 && $user['balance'] < $order_total) {
        $conn->rollBack();
        header("Location: ../cart.php?error=Số dư không đủ để thanh toán (bao gồm cả phí ship)");
        exit;
    }
    ...
    $conn->rollBack();
    header("Location: ../checkout.php?error=Có lỗi xảy ra: " . $e->getMessage());
    exit;
}
