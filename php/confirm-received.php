...
if (isset($_POST['order_id'])) {
    ...
    if ($order['status'] != 'shipped') {
        header("Location: ../order-detail.php?id=$order_id&error=Chỉ có thể xác nhận khi đơn hàng đã được giao");
        exit;
    }
    ...
    if ($stmt->execute([$order_id, $user_id])) {
        header("Location: ../order-detail.php?id=$order_id&success=Cảm ơn bạn đã xác nhận nhận hàng!");
    } else {
