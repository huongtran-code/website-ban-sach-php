...
if (isset($_POST['book_id']) && isset($_POST['book_title']) && isset($_POST['book_description']) && 
    ...
    if (empty($title) || empty($desc) || $author_id == 0 || $category_id == 0) {
        header("Location: ../edit-book.php?id=$id&error=Vui lòng điền đầy đủ tất cả các trường");
        exit;
    }

    if ($discount_percent < 0 || $discount_percent > 100) {
        header("Location: ../edit-book.php?id=$id&error=% Giảm giá phải từ 0-100");
        exit;
    }

    if ($return_days < 0 || $return_days > 365) {
        header("Location: ../edit-book.php?id=$id&error=Số ngày hoàn trả phải từ 0-365");
        exit;
    }
    ...
    if ($cover['error'] == 0) {
        ...
        @unlink("../uploads/cover/" . $old['cover']);
    ...
    header("Location: ../admin.php?success=Cập nhật sách thành công");
    exit;
} else {
