...
if (isset($_POST['id']) && isset($_POST['code']) && isset($_POST['discount_percent'])) {
    ...
    if (empty($code)) {
        header("Location: ../edit-coupon.php?id=$id&error=Vui lòng nhập mã khuyến mãi");
        exit;
    }

    if ($discount_percent < 1 || $discount_percent > 100) {
        header("Location: ../edit-coupon.php?id=$id&error=% giảm giá phải từ 1-100");
        exit;
    }
    ...
    if ($apply_type === 'category' || $apply_type === 'book') {
        if (isset($_POST['apply_to_ids']) && is_array($_POST['apply_to_ids']) && count($_POST['apply_to_ids']) > 0) {
            ...
            header("Location: ../edit-coupon.php?id=$id&error=Vui lòng chọn " . ($apply_type === 'category' ? 'danh mục' : 'sách') . " để áp dụng");
            exit;
        }
    }

    if ($usage_limit !== null && $usage_limit < 1) {
        header("Location: ../edit-coupon.php?id=$id&error=Số lượt sử dụng phải lớn hơn 0");
        exit;
    }

    if ($max_usage_per_user !== null && $max_usage_per_user < 1) {
        header("Location: ../edit-coupon.php?id=$id&error=Số lượt mỗi user phải lớn hơn 0");
        exit;
    }
    ...
    if ($code !== $existing_coupon['code']) {
        ...
        if ($check_stmt->fetch()) {
            header("Location: ../edit-coupon.php?id=$id&error=Mã khuyến mãi đã tồn tại");
            exit;
        }
    }
    ...
    try {
        if (update_coupon($conn, $id, $code, $description, $discount_percent, $apply_type, $apply_to_ids, $expires_at, $usage_limit, $max_usage_per_user)) {
            header("Location: ../admin-coupons.php?success=Sửa mã khuyến mãi thành công");
        } else {
            header("Location: ../edit-coupon.php?id=$id&error=Không thể cập nhật mã khuyến mãi");
        }
    } catch (PDOException $e) {
        header("Location: ../edit-coupon.php?id=$id&error=Lỗi: " . urlencode($e->getMessage()));
    }
    exit;
} else {
