...
if (isset($_POST['code'])) {
    ...
    if (empty($code)) {
        header("Location: ../../pages/admin-coupons.php?error=Vui lòng nhập mã khuyến mãi");
        exit;
    }

    if ($discount_type === 'percent' && ($discount_percent < 1 || $discount_percent > 100)) {
        header("Location: ../../pages/admin-coupons.php?error=% giảm giá phải từ 1-100");
        exit;
    }
    ...
    if ($apply_type === 'category' || $apply_type === 'book') {
        if (isset($_POST['apply_to_ids']) && is_array($_POST['apply_to_ids']) && count($_POST['apply_to_ids']) > 0) {
            ...
            header("Location: ../../pages/admin-coupons.php?error=Vui lòng chọn " . ($apply_type === 'category' ? 'danh mục' : 'sách') . " để áp dụng");
            exit;
        }
    }

    if ($usage_limit !== null && $usage_limit < 1) {
        header("Location: ../../pages/admin-coupons.php?error=Số lượt sử dụng phải lớn hơn 0");
        exit;
    }

    if ($max_usage_per_user !== null && $max_usage_per_user < 1) {
        header("Location: ../../pages/admin-coupons.php?error=Số lượt mỗi user phải lớn hơn 0");
        exit;
    }
    ...
    try {
        ...
        header("Location: ../../pages/admin-coupons.php?success=Thêm mã khuyến mãi thành công");
    } catch (PDOException $e) {
        if (strpos($e->getMessage(), 'Duplicate') !== false) {
            header("Location: ../../pages/admin-coupons.php?error=Mã khuyến mãi đã tồn tại");
        } else {
            header("Location: ../../pages/admin-coupons.php?error=Lỗi: " . urlencode($e->getMessage()));
        }
    }
    exit;
} else {
