...
if (isset($_POST['order_id'])) {
    ...
    if ($order['status'] != 'shipped' && $order['status'] != 'completed') {
        header("Location: ../../pages/order-detail.php?id=$order_id&error=Chỉ có thể hoàn trả khi đơn hàng đã được giao");
        exit;
    }
    ...
    if ($result['count'] == 0) {
        header("Location: ../../pages/order-detail.php?id=$order_id&error=Chỉ có thể hoàn trả sách bản cứng");
        exit;
    }
    ...
    if ($return_info) {
        ...
        if ($days_since_order > $min_return_days) {
            ...
            header("Location: ../../pages/order-detail.php?id=$order_id&error=" . urlencode($message));
            exit;
        }
    }
    ...
    if ($stmt->execute([$order_id, $user_id])) {
        ...
        header("Location: ../../pages/order-detail.php?id=$order_id&success=" . urlencode($message));
    } else {
