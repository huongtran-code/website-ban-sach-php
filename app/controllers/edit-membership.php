...
if (!isset($_POST['user_id']) || !isset($_POST['membership_level'])) {
    header("Location: ../../pages/admin-users.php?error=Thiếu thông tin");
    exit;
}
...
if (!in_array($membership_level, $valid_levels)) {
    header("Location: ../../pages/admin-users.php?error=Hạng thành viên không hợp lệ");
    exit;
}
...
try {
    ...
    header("Location: ../../pages/admin-users.php?success=Đã cập nhật hạng thành viên thành công");
    exit;
} catch (PDOException $e) {
    header("Location: ../../pages/admin-users.php?error=Lỗi: " . urlencode($e->getMessage()));
    exit;
}
