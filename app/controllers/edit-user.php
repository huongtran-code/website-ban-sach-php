...
if (isset($_POST['user_id']) && isset($_POST['full_name']) && isset($_POST['email'])) {
    ...
    if (empty($full_name) || empty($email)) {
        header("Location: ../../pages/edit-user.php?id=$id&error=Vui lòng điền đầy đủ thông tin bắt buộc");
        exit;
    }
    ...
    if ($stmt->fetch()) {
        header("Location: ../../pages/edit-user.php?id=$id&error=Email đã được sử dụng bởi người dùng khác");
        exit;
    }
    ...
    if (!empty($password)) {
        if (strlen($password) < 6) {
            header("Location: ../../pages/edit-user.php?id=$id&error=Mật khẩu phải có ít nhất 6 ký tự");
            exit;
        }
    ...
    header("Location: ../../pages/admin-users.php?success=Cập nhật người dùng thành công");
    exit;
} else {
