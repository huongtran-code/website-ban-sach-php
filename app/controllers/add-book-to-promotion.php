...
if (isset($_POST['promotion_id']) && isset($_POST['book_id']) && !empty($_POST['book_id'])) {
    ...
    if ($book_id <= 0) {
        header("Location: ../../pages/edit-promotion.php?id=$promotion_id&error=Vui lòng chọn sách");
        exit;
    }

    if ($custom_discount !== null && ($custom_discount < 1 || $custom_discount > 100)) {
        header("Location: ../../pages/edit-promotion.php?id=$promotion_id&error=% giảm giá phải từ 1-100");
        exit;
    }
    ...
    try {
        ...
        if ($existing) {
            header("Location: ../../pages/edit-promotion.php?id=$promotion_id&error=Sách này đã có trong chương trình khuyến mãi: " . htmlspecialchars($existing['promotion_name']));
            exit;
        }
        
        $result = add_book_to_promotion($conn, $promotion_id, $book_id, $custom_discount);
        if ($result) {
            header("Location: ../../pages/edit-promotion.php?id=$promotion_id&success=Thêm sách vào chương trình thành công");
        } else {
            header("Location: ../../pages/edit-promotion.php?id=$promotion_id&error=Không thể thêm sách. Vui lòng thử lại.");
        }
    } catch (Exception $e) {
