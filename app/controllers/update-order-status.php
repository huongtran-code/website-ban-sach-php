...
if (isset($_POST['order_id']) && isset($_POST['status'])) {
    ...
    if (!in_array($status, $allowed_statuses)) {
        header("Location: ../../pages/admin-order-detail.php?id=$order_id&error=Trạng thái không hợp lệ");
        exit;
    }
    ...
    try {
        ...
        if ($status == 'cancelled' && $old_status != 'cancelled') {
            if ($old_status != 'pending') {
                header("Location: ../../pages/admin-order-detail.php?id=$order_id&success=Hủy đơn hàng thành công! Đã hoàn tiền " . number_format($order['total_amount'], 0, ',', '.') . "đ cho khách hàng");
            } else {
                header("Location: ../../pages/admin-order-detail.php?id=$order_id&success=Hủy đơn hàng thành công (đơn hàng chưa thanh toán)");
            }
        } elseif ($status == 'returned') {
            header("Location: ../../pages/admin-order-detail.php?id=$order_id&success=Hoàn trả thành công! Đã refund " . number_format($order['total_amount'] * 0.85, 0, ',', '.') . "đ cho khách hàng (mất 15% phí)");
        } else {
            header("Location: ../../pages/admin-order-detail.php?id=$order_id&success=Cập nhật trạng thái thành công");
        }
    } catch (Exception $e) {
        $conn->rollBack();
        header("Location: ../../pages/admin-order-detail.php?id=$order_id&error=Có lỗi xảy ra: " . $e->getMessage());
    }
    exit;
} else {
    header("Location: ../../pages/admin-orders.php?error=Yêu cầu không hợp lệ");
    exit;
}
