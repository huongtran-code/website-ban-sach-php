...
if (isset($_POST['promotion_id']) && isset($_POST['book_id'])) {
    ...
    try {
        ...
        if ($stmt->execute([$custom_discount, $promotion_id, $book_id])) {
            header("Location: ../../pages/edit-promotion.php?id=$promotion_id&success=Cập nhật % giảm giá thành công");
        } else {
            header("Location: ../../pages/edit-promotion.php?id=$promotion_id&error=Không thể cập nhật");
        }
